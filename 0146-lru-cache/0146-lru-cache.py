#For O(1) get and 0(1) put we need to use a hashmap with key as the first number #of the pair and the value as the entire node pair. Now in order to have the least recent element move up the order, we use linked list and we use doublly linked list to move the last called element up to the head

#The keys are also maintained #as a doubly linked list with a dummy head and a dummy tail, which we use to move
#the last updated item to the top so the least recently used item stays on the
#bottom(qeue)

class DLL_Node():               #Class for Doubly Linked List Node
    def __init__(self):
        self.key=0
        self.value=0
        self.prev=None
...
class LRUCache:
    ...
    def __init__(self, capacity: int):
        self.capacity = capacity        #Capacity of LRU Cache
        self.head = DLL_Node()
        self.tail = DLL_Node()
